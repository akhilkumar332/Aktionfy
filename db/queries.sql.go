...
func (q *Queries) GetCountTracesBetween(ctx context.Context, arg GetCountTracesBetweenParams) (int64, error) {
	row := q.db.QueryRow(ctx, getCountTracesBetween, arg.StartTime, arg.StartTime_2)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const getLatestWorkflowState = `-- name: GetLatestWorkflowState :one
SELECT state_data FROM workflow_state WHERE task_id = $1 ORDER BY updated_at DESC LIMIT 1
`

func (q *Queries) GetLatestWorkflowState(ctx context.Context, taskID pgtype.UUID) ([]byte, error) {
	row := q.db.QueryRow(ctx, getLatestWorkflowState, taskID)
	var state_data []byte
...
func (q *Queries) GetSuccessRateBetween(ctx context.Context, arg GetSuccessRateBetweenParams) (float64, error) {
	row := q.db.QueryRow(ctx, getSuccessRateBetween, arg.StartTime, arg.StartTime_2)
	var column_1 float64
	err := row.Scan(&column_1)
	return column_1, err
}
...
type GetUserInvitationByTokenRow struct {
	ID          pgtype.UUID        `json:"id"`
	Email       string             `json:"email"`
	Role        string             `json:"role"`
	Tier        string             `json:"tier"`
...
func (q *Queries) GetUserInvitationByToken(ctx context.Context, inviteToken string) (GetUserInvitationByTokenRow, error) {
	row := q.db.QueryRow(ctx, getUserInvitationByToken, inviteToken)
	var i GetUserInvitationByTokenRow
	err := row.Scan(
		&i.ID,
		&i.Email,
		&i.Role,
...
func (q *Queries) GetWorkflowState(ctx context.Context, arg GetWorkflowStateParams) ([]byte, error) {
	row := q.db.QueryRow(ctx, getWorkflowState, arg.TaskID, arg.ExecutionID)
	var state_data []byte
	err := row.Scan(&state_data)
	return state_data, err
}
...
type ListExecutionTracesByExecutionIDParams struct {
	TaskID      pgtype.UUID `json:"task_id"`
	ExecutionID string      `json:"execution_id"`
}

func (q *Queries) ListExecutionTracesByExecutionID(ctx context.Context, arg ListExecutionTracesByExecutionIDParams) ([]ExecutionTrace, error) {
	rows, err := q.db.Query(ctx, listExecutionTracesByExecutionID, arg.TaskID, arg.ExecutionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListUserInvitations(ctx context.Context) ([]ListUserInvitationsRow, error) {
	rows, err := q.db.Query(ctx, listUserInvitations)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListUserInvitationsRow
	for rows.Next() {
...
type ListWorkspaceMembersRow struct {
	...
	Email         pgtype.Text `json:"email"`
	Role          pgtype.Text `json:"role"`
	WorkspaceRole pgtype.Text `json:"workspace_role"`
}
...
func (q *Queries) PruneZombieWorkers(ctx context.Context, dollar_1 interface{}) error {
...
func (q *Queries) SearchUserTasks(ctx context.Context, arg SearchUserTasksParams) ([]SearchUserTasksRow, error) {
	rows, err := q.db.Query(ctx, searchUserTasks,
		arg.UserID,
		arg.Search,
		arg.Status,
		arg.OffsetVal,
...
UPDATE workspaces SET name = COALESCE($1, name) WHERE id = $2 AND owner_id = $3 RETURNING id, name, owner_id, created_at
`
...
func (q *Queries) UpdateWorkspace(ctx context.Context, arg UpdateWorkspaceParams) (Workspace, error) {
	row := q.db.QueryRow(ctx, updateWorkspace, arg.Name, arg.ID, arg.OwnerID)
	var i Workspace
	err := row.Scan(
		&i.ID,
		&i.Name,
...
func (q *Queries) UpsertWorkflowState(ctx context.Context, arg UpsertWorkflowStateParams) error {
	_, err := q.db.Exec(ctx, upsertWorkflowState, arg.TaskID, arg.ExecutionID, arg.StateData)
	return err
}
