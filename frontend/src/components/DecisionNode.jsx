...
const DecisionNode = ({ data, selected }) => {
  ...
  const isTeam = task.task_type === 'swarm_router';
    ...
    <div className={`relative flex items-center justify-center transition-all duration-500 ${isProcessing ? 'scale-110' : ''}`}>
      ...
      <div 
        className={`
          ...
          ${selected ? `ring-2 ${isTeam ? 'ring-purple-500' : 'ring-indigo-500'} ring-offset-4 ring-offset-zinc-900` : ''}
          ${isProcessing ? (isTeam ? 'bg-purple-600/30 border-2 border-purple-400 shadow-[0_0_30px_rgba(168,85,247,0.4)]' : 'bg-indigo-600/30 border-2 border-indigo-400 shadow-[0_0_30px_rgba(99,102,241,0.4)]') : 'glass-surface'}
      ...
      >
        ...
        <div className="-rotate-45 flex flex-col items-center gap-1 text-center w-full px-2">
          <div className={`text-[8px] font-black uppercase tracking-widest ${isTeam ? 'text-purple-400' : 'text-indigo-400'}`}>
            {isTeam ? 'Team' : 'Decision'}
          </div>
          ...
          <div className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded ${
            ...
            task.status === 'processing' ? (isTeam ? 'bg-purple-500/20 text-purple-400 animate-pulse' : 'bg-indigo-500/20 text-indigo-400 animate-pulse') :
      ...
      {isProcessing && (
        <div className={`absolute inset-0 ${isTeam ? 'bg-purple-500/10' : 'bg-indigo-500/10'} rounded-full -z-10 animate-ping duration-[2000ms]`} />
      )}
      ...
      <Handle
        ...
        className={`${isTeam ? '!bg-purple-500' : '!bg-indigo-500'} !w-2 !h-2 !border-none`}
      />
      <Handle
        type="source"
