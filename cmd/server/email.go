...
func sendFailureEmail(ctx context.Context, userID string, taskID string, taskName string) {
	...
	from := mail.NewEmail("Aktionfy Server", "noreply@yourservice.com")
	subject := fmt.Sprintf("Action Required: Task '%s' Failed", taskName)
	to := mail.NewEmail("User", email.String)
	plainTextContent := fmt.Sprintf("Your aktionfy workflow '%s' (ID: %s) has failed 3 times and is now in an error state. Please review its configuration.", taskName, taskID)
	htmlContent := fmt.Sprintf("<strong>Your aktionfy workflow '%s'</strong> has failed 3 times and is now in an error state. Please log in to your dashboard to review its configuration.", taskName)
